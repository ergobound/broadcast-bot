# https://github.com/dikirilov/ptb-job-persistence-stackoverflow
...
logger.remove()
logger.add(
    sys.stdout, level="INFO", # level="TRACE"
    format="<green>{time:YYYY-MM-DD HH:mm:ss.SSS}</green> | <blue>{module}</blue> | <lvl>{level}</lvl> | {message}",
    serialize=False
)
...
def retrieve_state(job: Job):
    ...
    #         # Резервный вариант, если какого-то атрибута не хватает в структуре
    #         state = {
    #             'id': job.id,
    #             'func': job.func,
    #             'trigger': job.trigger,
